 ...
 *
 * <p>The Copilot is off by default for a deployment, so the flag is turned on here rather
 * than left to configuration: a suite that skips itself in CI tells nobody anything. The
 * gateway is left unset, which puts the panel on its built-in fixtures, so these run against
 * the real application and the real Fineract that CI already stands up, and need no model.
 */
...
test.describe('Copilot panel', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Turn the panel on for this run by rewriting the deployment config as it is served.
    // Setting window.env from an init script does not survive: assets/env.js is a plain
    // script that assigns the flag itself, and it runs after anything injected beforehand.
    await page.route('**/assets/env.js', async (route) => {
      const response = await route.fetch();
      const config = (await response.text())
        .replace(/\['enableCopilot'\]\s*=\s*[^;]+;/, "['enableCopilot'] = true;")
        .replace(/\['copilotMcpBaseUrl'\]\s*=\s*[^;]+;/, "['copilotMcpBaseUrl'] = 'mock';");
      await route.fulfill({ response, body: config, headers: { 'content-type': 'application/javascript' } });
    });

    await page.goto('/#/');

    // A safety net rather than the normal path: the officer also needs a permission the
    // Copilot allows, and a deployment could take that away.
    const present = await page
      .locator('button.ai-fab')
      .isVisible({ timeout: 30000 })
      .catch(() => false);
    test.skip(!present, 'The signed-in user holds no permission the Copilot allows.');
  });
