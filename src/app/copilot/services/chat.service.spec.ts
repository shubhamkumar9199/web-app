...
describe('ChatService', () => {
  ...
  /**
   * A decision is not a question. Offering the previous one back beside a restored card
   * would invite the officer to re-ask something unrelated to the action in front of them.
   */
  it('a failed decision does not offer an earlier question back', () => {
    mcpMock.chat.mockReturnValue(
      from([
        { type: 'action_card', pendingAction: { cardId: 'card-1', title: 'Approve', fields: [] } },
        { type: 'done' }
      ] as McpStreamEvent[])
    );
    service.sendMessage('how many loans does aisha have');
    ...
    mcpMock.decision.mockReturnValue(
      from([{ type: 'error', errorCode: 'AUTH_EXPIRED', message: 'Session expired', retryable: true }])
    );
    service.decideAction('approve');

    expect(service.messages$.value.some((message) => message.retryPrompt)).toBe(false);
  });
