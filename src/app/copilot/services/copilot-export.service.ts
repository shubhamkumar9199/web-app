...
export class CopilotExportService {
  ...
  private paginate(JsPdf: typeof jsPDF, canvas: HTMLCanvasElement): jsPDF {
    ...
    if (sliceHeightPx <= 0) {
      // Nothing was rendered, so there is nothing to paginate. Without this the loop below
      // advances by zero and never ends, which costs the officer the tab rather than the
      // export.
      return pdf;
    }
